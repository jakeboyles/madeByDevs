<div id="content" class="col-md-8 col-md-push-4">

	<h1><?php echo $main_team['name']; ?> vs. <?php echo $away_team['name']; ?></h1>


	<!-- Desktop Current Season Standings -->
	<div class="team-standings hidden-xs">
		<div class="team-header">
			<h3 class="team-name-primary"><?php echo $main_team['name']; ?> vs. <?php echo $away_team['name']; ?> </h3>
			<h3 class="team-name-secondary">History</h3>
			<!-- <h5 class="team-date">fall 2012</h5> -->
		</div>

		<table class="table table-striped stripe-pattern-one">
			<thead>
				<tr>
					<th>Winner</th>
					<th>Loser</th>
					<th>Score</th>
					<th>Date</th>

				</tr>
				<tr>
					<th colspan="6"><div class="dashed">&nbsp;</div></th>
				</tr>
			</thead>
			<tbody>
				<?php if(!empty($history)): ?>
				<?php foreach($history as $team): 
				?>
					<tr>
						<td>
							<?php if($team['win']=='1'):?>
								<a href="<?php echo base_url("teams/history")."/".$main_team['id']  ;?>"><?php echo $main_team['name']; ?></a>
							<?php elseif($team['tie']=='1'): ?>
								Tie
							<?php else: ?>
								<a href="<?php echo base_url("teams/history")."/".$away_team['id']  ;?>"><?php echo $away_team['name']; ?></a>
							<?php endif; ?>
						</td>
						<td>
							<?php if($team['win']=='1'):?>
								<a href="<?php echo base_url("teams/history")."/".$away_team['id']  ;?>"><?php echo $away_team['name']; ?></a>
							<?php elseif($team['tie']=='1'): ?>
								Tie
							<?php else: ?>
								<a href="<?php echo base_url("teams/history")."/".$main_team['id']  ;?>"><?php echo $main_team['name']; ?></a>
							<?php endif; ?>
						</td>
						<td><?php echo $team['score_home'] . "-" . $team["score_away"]; ?></td>
						<td><?php echo date('m/d/Y h:m A',strtotime($team['game_date_time'])); ?></td>
					</tr>
				<?php endforeach; ?>
				<?php endif; ?>
				
			</tbody>
		</table>
	</div>

	

	<!-- Team Roster (Mobile) -->
	<div class="mobile-expand-list visible-xs">
		<h3> History</h3>
		<ul class="content">
			<?php foreach( $history as $team ): ?>
			<li class="visible-xs">
				<ul class="visible-xs">
					<li class="visible-xs">
						<h5 class="title">Winner</h5>
						<h6 class="option">
							<?php if($team['win']=='1'):?>
								<a href="<?php echo base_url("teams/history")."/".$main_team['id']  ;?>"><?php echo $main_team['name']; ?></a>
							<?php elseif($team['tie']=='1'): ?>
								Tie
							<?php else: ?>
								<a href="<?php echo base_url("teams/history")."/".$away_team['id']  ;?>"><?php echo $away_team['name']; ?></a>
							<?php endif; ?>
						</h6>
						<h5 class="title">Loser</h5>
						<h6 class="option">
							<?php if($team['win']=='1'):?>
								<a href="<?php echo base_url("teams/history")."/".$away_team['id']  ;?>"><?php echo $away_team['name']; ?></a>
							<?php elseif($team['tie']=='1'): ?>
								Tie
							<?php else: ?>
								<a href="<?php echo base_url("teams/history")."/".$main_team['id']  ;?>"><?php echo $main_team['name']; ?></a>
							<?php endif; ?>
						</h6>
						<h5 class="title">Score</h5>
						<h6 class="option">
							<?php echo $team['score_home'] . "-" . $team["score_away"]; ?>
						</h6>
						<h5 class="title">Date</h5>
						<h6 class="option">
							<?php echo date('m/d/Y h:m A',strtotime($team['game_date_time'])); ?>
						</h6>
					</li>
				</ul>
			</li>
			<?php endforeach; ?>
		</ul>
	</div>



</div>